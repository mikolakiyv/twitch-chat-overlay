...
<img src="docs/screenshot_main3.png" width="440" alt="Оверлей: вкладки, поле «＋ ссылка или канал», ответ как на Twitch, награда за баллы, смайлы">

| Только текст (F9) | Настройки |
|:---:|:---:|
| <img src="docs/screenshot_textonly2.png" width="330"> | <img src="docs/settings-panel.png" width="300"> |
...
| Добавить канал по ссылке | вставьте любую ссылку Twitch (или имя) в поле «＋ ссылка или канал» справа над чатом, Enter |
...
| Прозрачный чат + поле ввода (писать и вставлять смайлы поверх игры) | **Insert** или настройки |
| Развернуть в большой мультичат (колонки) и обратно | **F10** или настройки |
| ↳ в развёрнутом виде колонками открываются только каналы **в эфире** (у живых в заголовке ●); если никто не стримит — все | |
...
Быстрый способ: скопируйте ссылку на канал (обычную, попаут-чата, мод-вью —
любую) и вставьте в поле **«＋ ссылка или канал»** справа над чатом, Enter —
чат откроется новой вкладкой. Если канал уже открыт, просто переключит на него.

Во вкладке «Все» перед каждым сообщением стоит **круглая иконка канала**
(аватарка стримера) — как в Twitch. Не нравится — в настройках «Иконки
каналов в ленте» выключается, и вернутся текстовые метки `#канал`.

Если каналы ведут совместный стрим с общим чатом (Twitch Shared Chat), Twitch
присылает каждое сообщение во все каналы-участники. Во вкладке «Все» оверлей
показывает такое сообщение один раз — из канала, где его написали; во
вкладках самих каналов оно есть у каждого, как на сайте, и у каждого
сообщения стоит аватарка канала, где его написали (включая свой) — даже если
другой канал у вас не подключён.

Или в настройках перечислите каналы через запятую. Появятся **вкладки**:
...
В развёрнутом мультичате (**F10**) колонки строятся только из каналов,
которые **сейчас в эфире** — удобно, когда в списке много избранных, а
стримят двое. У живых каналов в заголовке колонки горит метка ●. Если не
стримит никто (или статус ещё не загрузился), показываются все каналы.
Статус обновляется при каждом нажатии F10 и дальше раз в 15 минут.

<img src="docs/screenshot_multichat.png" width="640" alt="Развёрнутый мультичат: колонки только живых каналов с метками ●">
...
- **Захват окна** (обычный случай) — Windows отдаёт OBS «сырое» окно, поэтому
  ...
  оверлея включите прозрачный режим («Прозрачный фон», **F9** или **Insert**) и
  **«Хромакей для OBS»** — фон захвата станет ровным тёмным цветом `#0b0b10`
  (на вашем экране ничего не изменится). Затем в OBS: правый клик по источнику →
  **«Фильтры» → «+» → «Цветовой ключ»**, тип цвета — «Свой цвет» `#0b0b10`,
  сходство **10**, сглаживание **5**. Цвет тёмный намеренно: буквы сглаживаются
  по фону, и с ярким ключом (зелёным, пурпурным) текст получает цветную кайму,
  которую фильтр не вырезает; с тёмным кайма выглядит как тонкая обводка. Готово,
...
- **Ответы как на Twitch**: над ответом — строка «↩ Ответ @ник: исходное
  сообщение» (ник кликабельный), а сам текст ответа показывается как есть,
  с «@ник» в начале — сразу видно, кто кому пишет.
- **Награды за баллы канала**: над сообщением — иконка награды, «ник
  забирает награду «название»» и цена со значком баллов канала. Работают и
  встроенные награды («Выделить сообщение» — с фиолетовой подсветкой,
  «Увеличенный смайл», «Эффект сообщения»). Логин не нужен.
- **Первое сообщение** зрителя в чате выделяется как в 7TV: розовая кромка
  слева, подложка и метка «ПЕРВОЕ СООБЩЕНИЕ» справа сверху.
- Всё это отключается одним тогглом в настройках («Ответы, награды, первые
  сообщения»).
...
Оверлей обновляется сам. При запуске и дальше раз в шесть часов он
спрашивает GitHub, вышла ли новая версия; если вышла — тихо скачивает её
рядом с собой, сверяет размер и контрольную сумму и **ставит при следующем
запуске**. Стрим никто не перезапустит: посреди работы ничего не меняется.
Не хочется ждать — в шапке появляется кнопка **⟳ v1.xx**: клик, и оверлей
перезапустится уже новой версией. Никаких прав администратора не нужно —
программа подменяет свой же файл на рабочем столе.

Файл можно переименовать или оставить как скачал браузер («TwitchChatOverlay
(2).exe») — обновление сохранит имя.

Отключить фоновое скачивание можно в настройках («Автообновление»); тогда
оверлей только сообщит о новой версии, а скачает и поставит по клику ⟳.
Там же есть «Проверить обновления» и номер текущей версии.
