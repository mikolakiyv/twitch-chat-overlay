...
Если вы модератор канала (или это ваш канал) и вы вошли в аккаунт —
**правый клик по нику** в чате открывает карточку действий: открыть профиль,
удалить сообщение, таймаут 10 минут, бан (с подтверждением). Кнопки модерации
видны только на каналах, где у вас есть модерка.

Важно: токену нужны модерские права. На twitchtokengenerator.com при выпуске
токена отметьте дополнительно `moderator:manage:banned_users` и
`moderator:manage:chat_messages` (если токен уже был — просто перелогиньтесь
с новым токеном).
